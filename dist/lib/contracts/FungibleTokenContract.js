...
export class FungibleTokenContract extends Contract {
     ...
     */
    transfer(_, input) {
        const { amount, ownerAddress, balance, recipientAddress, recipientBalance, } = input;
        if (!balance)
        ...
        balances[recipientAddress] = BigInt(BigInt(balances[recipientAddress] ?? '0') + BigInt(amount)).toString();
