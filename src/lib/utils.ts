 ...
 */
export function formatAmountToHex(input: number | string): string {
  try {
    ...
    if (typeof input === 'string' && input.startsWith('0x')) {
      ...
      const decimalNumber =
        typeof input === 'number' ? input : parseFloat(input)
 ...
 */
export function formatHexToAmount(
  hexString: string,
  units: number = 18
): string {
  try {
...
export function formatVerse(input: BigInt | string): string {
  ...
  if (typeof input === 'string') {
    ...
    bigIntValue = input as bigint
  }

  const divisor = BigInt('1000000000000000000')
  const wholePart: BigInt = bigIntValue / divisor
